...
pub async fn start_server(port: u16, ready_tx: Option<std::sync::mpsc::Sender<()>>) -> anyhow::Result<()> {
    ...
    let app = Router::new()
        ...
        .route("/api/pick-cache-file", post(scan::api_pick_cache_file))
        ...
        .route("/api/file", get(misc::api_file))
        .route("/api/pick-folder", get(misc::api_pick_folder))
        .route("/api/pick-save-zip", get(misc::api_pick_save_zip))
        .route("/api/pick-open-zip", get(misc::api_pick_open_zip))
        .route("/api/preset/save", post(misc::api_save_preset))
        .route("/api/preset/load", get(misc::api_load_preset))
